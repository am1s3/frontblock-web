...
export default function Admin() {
  ...
  const postNews = async (e: React.FormEvent) => { e.preventDefault(); const r = await api("/api/admin/news", { method: "POST", body: JSON.stringify(nt) }); setMsg(r.ok ? "✓ сводка опубликована" : (r.data?.error || "ошибка")); if (r.ok) setNt({ title: "", body: "" }); };

  if (loading) return <div className="muted">проверка допуска командира…</div>;
  if (!isAdmin) return <Panel label="ДОСТУП ЗАПРЕЩЁН"><p className="msg err">Только для командования. Твой аккаунт не помечен как админ.</p></Panel>;

  return (
    <>
      <h1 className="page-h">Командный пункт <small>// админка</small></h1>
      {msg && <p className="msg ok" style={{ marginBottom: 8 }}>{msg}</p>}
      <Panel label="ЛИЧНЫЙ СОСТАВ">
        <table className="tbl"><thead><tr><th>Боец</th><th>Ранг</th><th>Фр.</th><th>K/D</th><th>¤</th><th>Статус</th><th>Действия</th></tr></thead>
          <tbody>{players.map((p) => (
            <tr key={p.nickname}>
              ...
              <td style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                <button className="btn btn--ghost btn--sm" onClick={() => act(p.is_banned ? "/api/admin/unban" : "/api/admin/ban", { nickname: p.nickname, reason: "нарушение устава" })}>{p.is_banned ? "Разбан" : "Бан"}</button>
                ...
                <button className="btn btn--ghost btn--sm" onClick={() => { const v = prompt("Фракция TPA/VSS (пусто = разлок)", p.faction || ""); if (v !== null) act("/api/admin/setfaction", { nickname: p.nickname, value: v.toUpperCase() || null }); }}>Фракция</button>
              </td>
            </tr>
          ))}</tbody></table>
      </Panel>
      <Panel label="ПУБЛИКАЦИЯ СВОДКИ">
