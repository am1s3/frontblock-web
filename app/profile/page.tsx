...
export default function Profile() {
  ...
  if (authLoading) return <div className="muted">проверка допуска…</div>;
  if (err) return <Panel label="ДОСТУП"><p className="msg err">{err}</p></Panel>;
  if (!me) return <div className="muted">загрузка досье…</div>;
  ...
  return (
    <>
      <h1 className="page-h">Досье <small>// {me.nickname}</small></h1>
      <Reveal>
        <Panel label="ЛИЧНОЕ ДЕЛО" accent={me.faction === "TPA" ? "tpa" : me.faction === "VSS" ? "vss" : undefined}>
          <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "baseline" }}>
            <div><div className="muted" style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>ПОЗЫВНОЙ</div><h2 style={{ fontSize: "2rem" }}>{me.nickname}</h2></div>
            <div><div className="muted" style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>ЗВАНИЕ</div><div style={{ fontSize: "1.3rem", color: "var(--amber)" }}>Ранг {me.rank}</div></div>
      ...
      <Reveal delay={80}>
        <div className="grid-4">
          ...
          <Panel><StatBlock value={me.vehicles_destroyed} label="Техники" accent="vss" /></Panel>
          <Panel><StatBlock value={me.money} label="Монеты ¤" accent="amber" /></Panel>
          <Panel><StatBlock value={h} label="Часов в бою" suffix={`ч ${m}м`} /></Panel>
          <Panel><StatBlock value={me.is_admin ? 1 : 0} label={me.is_admin ? "Командир" : "Рядовой"} /></Panel>
        </div>
      </Reveal>
    </>
  );
